...
| `fix/v0.3.6-beta.12-issues-12-14-15-20` | short-lived | integrated; cleanup pending | `dev` | `dev` | Resolve beta.12 placeholder efficiency, operational logging, obsolete settings, and automatic reliability-action clarity. |
...
- `fix/v0.3.6-beta.12-issues-12-14-15-20`: integrated into `dev` at `9bec63a1d72082e1efffa42bd0c758dcf3bf29dd` and published as immutable tag `v0.3.6-beta.12`. The exact tag archive passed 151 tests, Python compilation, archive-layout and version checks, workspace reconciliation, and governed-project validation. Registry `dev` commit `c4bd327791428202e8bc7df3f4551272c2876cec` passed GitHub manifest validation. Dispatcharr's managed update reports `0.3.6-beta.12`, `loaded=true`, and `update_available=false`; the first scheduled beta.12 scan completed 1,397 streams with zero capacity deferrals and validated the one-second known-placeholder path plus runtime/dead-breakdown logging. Issues #12, #14, #15, and #20 are closed with this evidence; #21 is a separate follow-up. The remote branch remains intentionally retained pending explicit cleanup approval.
