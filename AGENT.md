...
- Retain terminal direct-throughput observations for 90 days, including prior/current status and exact `media_changed` source and cause. The 500 Kbps floor is never a media-change delta.
- Retain applied-sort movement details for 90 days and daily sort totals for 365 days. Dry runs preserve this history but do not count as actual ordering changes.
